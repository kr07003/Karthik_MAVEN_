...
public class App 
{
    public static void main( String[] args )
    {
        ...
        for(int i=0;i<n;i++)
        {
        	Cadbury nutties=new Cadbury();
        	Nestle kitkat=new Nestle();
        	kisses kiss=new kisses(i+1);
        	Jamun jamun=new Jamun(i+1);
        	Motichoor motichoor=new Motichoor();
        	Kheer kheer=new Kheer();
        	Gift x=new Gift();
        	x.addChocolate(nutties);
        	x.addChocolate(kitkat);
        	x.addChocolate(kiss);
        	x.addSweets(jamun);
        	x.addSweets(motichoor);
        	x.addSweets(kheer);
        	gift.add(x);
        }
        int weight=0;
